...
from baseline_dataset_loader_utils import load_image_paths_labels,  ImageDataset
from feature_extractor import get_processor
...
class BaseLineModel(nn.Module):
    '''
    backbone and neck are feature extractors, the difference is backbone's fixed while neck's learnable
    head is the classifier and it's learnable
    '''
...
def train_baseline(model, dataset, batch_size=32, epochs=50, opt_lr=0.001):
    '''
    ...
    # filter parameters before passing to optimizer
...
if __name__ == '__main__':
    ...
    # baseline 1
    baseline1_model = BaseLineModel(backbone=backbone, head=BL1_HEAD(input_dim=input_dim, output_dim=8)).to(device)
